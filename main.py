...
def cmd_status():
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Agent Engine Core Telemetry & Status[/bold cyan]", border_style="cyan"
        )
    )
    ...
    table.add_row(
        "llama.cpp / vLLM Server", "http://localhost:8080/v1", f"{llamacpp_color}{llamacpp_state}"
    )
    ...
    for provider, name in [
        ...
        has_key = status.get(f"has_{provider}_key", status.get(f"{provider}_key_present", False))
        table.add_row(
            name,
            "Configured" if has_key else "Not Set",
            "[green]Ready" if has_key else "[dim]Unset",
        )
    ...
    try:
        ...
        table.add_row(
            "Sentry Staged Patches",
            f"{staged_count} pending reviews",
            "[green]Clean" if staged_count == 0 else "[yellow]Pending Review",
        )
    except Exception:
...
def cmd_demo():
    ...
    for field_name, field_def in sig.fields.items():
        console.print(
            f" - Field: [bold green]{field_name}[/bold green] ({type(field_def).__name__}) -> {field_def.json_schema_extra.get('desc')}"
        )
...
def main():
    ...
    subparsers.add_parser("staged", help="List all pending staged patches")
    ...
    if args.command == "status":
        ...
        console.print(
            "[bold cyan]🚀 Launching Streamlit Control Center on http://localhost:8501...[/bold cyan]"
        )
        subprocess.run(
            [
                sys.executable,
