...
class NodeConfig(BaseModel):
    ...
    description: str = Field(default="Perform task logic according to inputs and outputs.", description="Agent objective/instructions")
    inputs: Union[List[str], Dict[str, str]] = Field(default_factory=list, description="Input fields list or {field_name: field_desc}")
    outputs: Union[List[str], Dict[str, str]] = Field(default_factory=list, description="Output fields list or {field_name: field_desc}")
    tools: List[str] = Field(default_factory=list, description="List of registered tool names")
    reasoning_type: str = Field(default="cot", description="Reasoning type: 'cot' (ChainOfThought), 'react' (ReAct), or 'predict'")
...
class AgentPipeline(dspy.Module):
    ...
    def forward(self, **initial_inputs) -> Dict[str, Any]:
        ...
        for idx, agent in enumerate(self.steps):
            ...
            agent_inputs = {}
            inp_keys = agent.config.inputs if isinstance(agent.config.inputs, list) else list(agent.config.inputs.keys())
            for key in inp_keys:
                if key in state:
